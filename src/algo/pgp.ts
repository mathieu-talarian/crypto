...
export class PGP extends AlgoBase {
  ...
  generateGPGKey = async () => {
    this.privateKey = new TextDecoder().decode(
      toByteArray(import.meta.env.VITE_GPG_PRIVATE)
    );
    this.publicKey = new TextDecoder().decode(
      toByteArray(import.meta.env.VITE_GPG_PUBLIC)
    );
