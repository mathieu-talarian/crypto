...
export class Nacl extends AlgoBase {
	...
	encrypt = (message: string, iv: Uint8Array): Promise<Uint8Array> => {
		const encoded = new TextEncoder().encode(message);
		return Promise.resolve(nacl.secretbox(encoded, iv, this.cryptoPrivateKey));
	};

	decrypt = (cypher: Uint8Array, iv: Uint8Array): Promise<string> => {
		const encoded = nacl.secretbox.open(cypher, iv, this.cryptoPrivateKey);
