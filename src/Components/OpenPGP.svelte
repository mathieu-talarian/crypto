<script lang="ts">
  ...
  const encodeOneMB = async () => {
    const encrypted = await pgp.encrypt(JSON.stringify(oneMB));
  ...
  const decodeOneMB = async () => {
    ...
    const decrypted = await pgp.decrypt(encoded);
  ...
  const decodeFiveMB = async () => {
    ...
    const decrypted = await pgp.decrypt(fiveEncrypted);
...
<header class="bg-white shadow">
  <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold text-gray-900">Open PGP</h1>
  </div>
</header>
<main>
